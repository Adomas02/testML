...
smells = ['isMysteryGuestManual', 'isResourceOptimismManual', 'isTestRedundancyManual']
...
for smell in smells:
    ...
    for dropout in dropout_rates:
        for epochs in epochs_list:
            ...
            history = model.fit(X_train, y_train, epochs=epochs, batch_size=32, validation_split=0.1, class_weight=class_weight)
            ...
            print(f'{smell} - Epochs: {epochs}, Dropout: {dropout} - Loss: {loss:.4f}, Accuracy: {accuracy:.4f}, F1-score: {f1:.4f}')

            # Save results
            with open(f'results_{smell}_epochs{epochs}_dropout{dropout}.txt', 'w') as f:
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(f'loss_{smell}_epochs{epochs}_dropout{dropout}.png')
            plt.close()
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(f'accuracy_{smell}_epochs{epochs}_dropout{dropout}.png')
            plt.close()
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(f'roc_curve_{smell}_epochs{epochs}_dropout{dropout}.png')
            plt.close()

            model.save(f'model_{smell}_epochs{epochs}_dropout{dropout}.keras')
